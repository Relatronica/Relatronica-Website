...
export function GanttChart({ deadlines, selectedDeadline, onSelectDeadline }: GanttChartProps) {
  ...
  const timeRange = useMemo(() => {
    ...
    const starts = deadlines.map(d => getHorizonStart(d).getTime());
    const ends = deadlines.map(d => getHorizonEnd(d).getTime());
  ...
  const yearSegments = useMemo(() => {
    ...
    return yearMarkers.map((marker, idx) => {
      ...
      const width = nextMarker
        ? Math.max(0, nextMarker.position - marker.position)
        : Math.max(0, 100 - marker.position);
      return {
        year: marker.year,
              ...
              {sortedDeadlines.map(deadline => {
                ...
                return (
                  <div
                    key={deadline.id}
                    onClick={() =>
                      onSelectDeadline(deadline.id === selectedDeadline ? null : deadline.id)
                    }
                    className={cn(
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                        ...
                        {formatDeadlineHorizonShort(deadline, 'it')}
                      </div>
                    </div>
                    <div
                      className={cn(
              ...
              <div className="pb-6" style={{ width: `${timelineWidth}px` }}>
                ...
                <div className="relative mt-4" style={{ height: `${sortedDeadlines.length * 60}px` }}>
                  ...
                  {sortedDeadlines.map((deadline, index) => {
                    ...
                    const spanWidthPct = Math.max(endPos - startPos, 0.8);
                    const isWindow = Boolean(deadline.horizonEnd) || deadline.precision === 'window';
                    const isPast = isDeadlinePast(deadline);
                    const isSelected = selectedDeadline === deadline.id;
                    const rowHeight = 60;
                    const top = index * rowHeight;
                    const minBarPx = isWindow ? 80 : 220;
                    const barWidthPct = isWindow
                      ? Math.max(spanWidthPct, (minBarPx / timelineWidth) * 100)
                      : Math.max((minBarPx / timelineWidth) * 100, 2);
                    ...
                    return (
                      ...
                      >
                        <div
                          ...
                          className={cn(
                            'absolute left-0 top-0 h-full rounded-lg cursor-pointer transition-all group shadow-sm',
                        ...
                        >
                          <div className="absolute left-0 top-0 h-full flex items-center pl-3 pr-3 pointer-events-none overflow-hidden">
                            <div className="text-xs font-medium text-white drop-shadow-md line-clamp-1">
                              {deadline.title}
                            </div>
                          </div>
