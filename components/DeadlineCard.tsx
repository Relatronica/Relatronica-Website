...
export function DeadlineCard({ deadline, variant = 'default' }: DeadlineCardProps) {
  ...
  const horizonLabel = formatDeadlineHorizon(deadline, locale);
  ...
  const natureChip = deadline.nature
    ? (() => {
        const key = natureLabelKey(deadline.nature);
        const label = t(key);
      ...
      <div>
        <div className="mb-4">
          <div className="flex flex-wrap items-center gap-2 mb-3">
            ...
            {natureChip && (
              <div className="inline-flex px-2.5 py-1 rounded-full text-[11px] font-medium bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
                {natureChip}
              </div>
            )}
          </div>
