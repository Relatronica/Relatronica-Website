...
const COLOR_MAP: Record<string, { icon: string; bg: string; border: string; gradient: string }> = {
  blue: { icon: 'text-blue-600', bg: 'bg-blue-50', border: 'border-blue-200', gradient: 'from-blue-50 to-indigo-50' },
  purple: { icon: 'text-purple-600', bg: 'bg-purple-50', border: 'border-purple-200', gradient: 'from-purple-50 to-violet-50' },
  emerald: { icon: 'text-emerald-600', bg: 'bg-emerald-50', border: 'border-emerald-200', gradient: 'from-emerald-50 to-teal-50' },
  orange: { icon: 'text-orange-600', bg: 'bg-orange-50', border: 'border-orange-200', gradient: 'from-orange-50 to-amber-50' },
};
        ...
        <div className="min-h-screen pt-32 pb-24 px-6">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="text-center mb-20">
              <div className="inline-flex items-center justify-center p-4 rounded-2xl bg-slate-100 mb-6">
              ...
              <h1 className="text-5xl md:text-6xl font-bold text-slate-900 leading-tight mb-4">
                Relatronica <span className="text-blue-600">Academy</span>
              </h1>
              ...
              {completedCount > 0 && (
                <div className="mt-6">
                  <div className="inline-flex items-center gap-3 px-5 py-2.5 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-700 text-sm font-medium">
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-col md:flex-row md:items-start gap-6">
                        <div className={`flex-shrink-0 w-16 h-16 rounded-2xl bg-white/80 flex items-center justify-center shadow-sm`}>
                        ...
                        <div className="flex-shrink-0 self-center">
                          <span className="inline-flex items-center gap-2 px-5 py-2.5 bg-white/90 border border-slate-200 rounded-full text-sm font-semibold text-slate-900 group-hover:bg-slate-900 group-hover:text-white group-hover:border-slate-900 transition-all duration-200 shadow-sm">
                ...
                <div className="grid md:grid-cols-3 gap-6">
                  {comingSoonCourses.map((course) => {
                    ...
                    return (
                      <div
                        key={course.slug}
                        className="rounded-2xl border border-slate-200 bg-white/90 backdrop-blur-xl p-6 shadow-sm opacity-80"
                      >
                        ...
                        <span className="inline-block px-3 py-1 rounded-full bg-slate-100 text-slate-500 text-xs font-medium">
                          {t('common.comingSoon')}
                        </span>
                      </div>
                    );
            ...
            <div className="text-center">
              <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-50 to-blue-50 p-10 shadow-sm max-w-2xl mx-auto">
                <h2 className="text-2xl font-bold mb-4 text-slate-900">{t('learn.cta.title')}</h2>
                <p className="text-slate-600 mb-6 text-[15px] leading-relaxed">
                  {t('learn.cta.description')}
                </p>
                <Link
                  href="/manifesto"
                  className="inline-flex items-center gap-2 px-6 py-3 bg-slate-900 text-white rounded-full font-semibold hover:bg-slate-800 transition-all duration-200 shadow-md hover:shadow-lg text-sm"
                >
                  {t('learn.cta.button')}
                  <ArrowRight className="w-4 h-4" />
                </Link>
