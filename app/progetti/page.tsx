...
export default function ProgettiPage() {
  ...
  const getStatusStyle = (status: string) => {
    ...
    if (status === active) return 'bg-emerald-100 text-emerald-700 border-emerald-200 dark:bg-emerald-950/50 dark:text-emerald-400 dark:border-emerald-800';
        ...
        <div className="min-h-screen pt-32 pb-24 px-6">
          <div className="max-w-4xl mx-auto">
            <FadeIn className="text-center mb-16">
              <h1 className="text-5xl sm:text-6xl font-bold mb-6 text-slate-900 dark:text-white">{t('projects.title')}</h1>
            ...
            <FadeIn delay={0.3}>
              <div className="divide-y divide-slate-200 dark:divide-slate-800 border-y border-slate-200 dark:border-slate-800">
                ...
                {filteredProgetti.map((progetto) => {
                  ...
                  const RowContent = (
                    <div className={`flex flex-col sm:flex-row sm:items-center gap-4 py-5 px-4 -mx-4 rounded-xl transition-all duration-200
                    ...
                    >
                      <div className="flex items-start gap-3 sm:w-2/5 shrink-0">
                        ...
                        <div className="min-w-0">
                          <h3 className="text-base font-bold text-slate-900 dark:text-white leading-tight">{progetto.title}</h3>
                          <p className="text-sm text-slate-500 dark:text-slate-500 mt-0.5">{progetto.subtitle}</p>
                        </div>
                      </div>
                  ...
                  if (isComingSoon) return <div key={progetto.id}>{RowContent}</div>;
                  return <Link key={progetto.id} href={progetto.href!} className="block">{RowContent}</Link>;
                })}
              </div>
            </FadeIn>
            ...
            <FadeIn delay={0.4} className="mt-20 text-center">
              <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white/90 dark:bg-slate-900/80 backdrop-blur-xl p-10 shadow-sm max-w-2xl mx-auto">
                <h2 className="text-2xl font-bold mb-4 text-slate-900 dark:text-white">{t('projects.contact.title')}</h2>
                <p className="text-slate-600 dark:text-slate-400 mb-6 text-[15px] leading-relaxed">
                  {t('projects.contact.description')}
                </p>
                <p className="text-sm text-slate-500 mb-4">{t('projects.contact.cta')}</p>
